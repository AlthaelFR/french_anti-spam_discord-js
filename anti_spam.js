    ...
    for (var i = 0; i < messageLog.length; i++) {
      ...
      if (user) {
        user.ban(deleteMessagesAfterBanForPastDays).then((member) => {
            ...
            .setAuthor(m.author.id, m.author.avatarURL)
            .setDescription(m.author.tag+ " a été bannis pour avoir spam.")
            .setColor('#'+Math.floor(Math.random()*16777215).toString(16))
            .setTimestamp();
          m.channel.send(banEmbed);
          return true;
       }).catch(() => {
          m.channel.send(`Il semblerait que je n'ai pas les permissions nécessaire pour bannir ${m.author.tag} !`);
          return false;
      });
    }
  }
